...
# to one-hot vector with spatial dimensions
def to_categorical(y, num_columns):
    """Returns one-hot encoded Variable with shape (B, 10, 1, 1)"""
...
fix_noise = torch.randn(10, opt.dim_noise, 1, 1, device=device).repeat_interleave(10, dim=0)
...
for epoch in range(opt.n_epochs):
    ...
    for i, batch in enumerate(dataloader, 0):
        ...
        # 4. Input generator_inputs to Generator to get fake data, and input fake data to netD to get predition, then calculate the rake_loss = -log(1 - D(G(z))) and backward
        # 5. OptimD.step()
        ############################            
